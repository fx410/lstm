...
for i in range(len(train) - window_len):
	  ...
	  train_in.append(tmp)
train_out = (train['high'][window_len:].values / train['high'][:-window_len].values)
...
for i in range(len(test) - window_len):
	  ...
	  for col in test:
			  tmp.loc[:,col] = tmp[col] / tmp[col].iloc[0] - 1
	  test_in.append(tmp)
test_out = (test['high'][window_len:].values / test['high'][:-window_len].values)
...
open('usd_high_model.json', 'w').write(model_json)
yen_model.save_weights('usd_high_weights.h5')

 #モデルによる予測と実測の比較
fig, ax1 = plt.subplots(1,1)
ax1.plot(df[df['time']< split_date]['time'][window_len:].astype(datetime),train['high'][window_len:],label='Actual', color='blue')
ax1.plot(df[df['time']< split_date]['time'][window_len:].astype(datetime),((np.transpose(yen_model.predict(train_in))) * train['high'].values[:-window_len])[0],label='Predicted', color='red')
plt.show()

fig, ax1 = plt.subplots(1,1)
ax1.plot(df[df['time']>= split_date]['time'][window_len:].astype(datetime),test['high'][window_len:], label='Actual', color='blue')
ax1.plot(df[df['time']>= split_date]['time'][window_len:].astype(datetime),((np.transpose(yen_model.predict(test_in)))*test['high'].values[:-window_len])[0],label='Predicted', color='red')
ax1.grid(True)
plt.show()
