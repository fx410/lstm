...
def date_to_str(date):
 ...
 #為替のデータ
res = oanda.get_history(instrument="EUR_JPY", granularity="D",count='5000')
res = pd.DataFrame(res['candles'])
...
yen_model = build_model(train_in, output_size=1, neurons=20)
 ...
 #モデルの学習
yen_history = yen_model.fit(train_in, train_out, epochs=15, batch_size=1, verbose=2, shuffle=True)

model_json = yen_model.to_json()
open('eur_model.json', 'w').write(model_json)
yen_model.save_weights('eur_weights.h5')
